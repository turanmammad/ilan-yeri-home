...
const ProductCard = ({ product, index }: { product: Product; index: number }) => (
  <motion.article
    ...
    className="group bg-card rounded-2xl border border-border overflow-hidden cursor-pointer hover:shadow-card-hover hover:-translate-y-1 transition-all duration-200"
  >
    <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
      ...
      <button className="absolute top-2.5 right-2.5 w-9 h-9 bg-card/80 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-card shadow-sm">
        <Heart className="w-4.5 h-4.5 text-foreground/60" strokeWidth={1.5} />
      </button>
    </div>
    <div className="p-3.5">
